/*
  # Network runs — the distributed replication fleet's filings

  The Replication Network's data store. A "run" is one participant's
  environmental profile for a campaign: how noisy their rig is, its mains
  frequency, its dominant vibration line. Campaign census-001 is the
  calibration round — before any replication of a famous claim, the fleet
  establishes its collective sensitivity floor (median noise / √N).

  Units: noise_rms and top_peak_g are in milli-g. No geolocation, ever:
  a device label is optional free text (≤ 40 chars), everything else is
  physics. Access model follows claim_registry: public read, bounded
  anonymous insert, no delete — filings are a record.
*/
