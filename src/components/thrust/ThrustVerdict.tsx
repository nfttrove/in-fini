...
export default function ThrustVerdict({ budget }: Props) {
  const tone = THRUST_TONE_CLASSES[budget.verdict.tone];
  const Icon =
  ...
  const pct = Math.max(
    ...
    Math.min(
      100,
      budget.claimedG !== 0
        ? (budget.residualG / Math.abs(budget.claimedG)) * 100
        : 0
    )
  );

  return (
    <Panel title="Thrust Verdict">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="dark-mode:bg-slate-900 light-mode:bg-slate-100 coffee-mode:bg-slate-900 rounded-lg p-3">
            ...
            <div className="font-mono text-base font-semibold text-amber-300">
              {formatForceG(budget.claimedG)}
            </div>
          </div>
          <div className="dark-mode:bg-slate-900 light-mode:bg-slate-100 coffee-mode:bg-slate-900 rounded-lg p-3">
            <div className="text-xs dark-mode:text-slate-500 light-mode:text-slate-600 coffee-mode:text-slate-500 mb-1">Summed artifacts</div>
